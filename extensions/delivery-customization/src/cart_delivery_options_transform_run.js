...
export function cartDeliveryOptionsTransformRun(input) {
  ...
  targets.forEach((variant) => {
    variant.product.hasTags.forEach((tagResponse) => {
      switch (tagResponse.tag) {
        case "Restricted For London":
          if (tagResponse.hasTag) resLondon = true;
          break;
        case "Restricted For UK Wide":
          if (tagResponse.hasTag) resUKWide = true;
          break;
      }
    });
  });
 ...
 const deliveryGrp = {
  ...
  var delivery_grp = getDeliveryGroup(zip);
...
function zip_delivery_option(
  ...
  if (delivery_grp == "delivery_1") {
    deliveryBool = london_delivery(storeDate, formattedTime, deliveryDate, deliveryDay, tomorrow, resLondon);
  }
  else {
    deliveryBool = uk_wide_delivery(storeDate, deliveryDate, deliveryDay, formattedTime, tomorrow, resUKWide);
  }
  return deliveryBool;
}
...
function uk_wide_delivery(storeDate, deliveryDate, deliveryDay, formattedTime, tomorrow, resUKWide) {
  const options = {
  ...
  } else if (storeDate != deliveryDate){
    deliveryOptions.push(options.deliveredMorning);
  }
